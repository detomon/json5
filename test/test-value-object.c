...
int main (int argc, char const * argv [])
{
	...
	item = json5_value_set_prop (&value, "akey34", 6, 0);
	...
	item2 = json5_value_set_prop (&value, "akey34", 6, 1);
	assert (item2 != NULL);
	assert (item2 == item);
	assert (item2 -> type == JSON5_TYPE_NULL);
	assert (value.len == 1);

	item2 = json5_value_set_prop (&value, "akey34", 6, 0);
	assert (item2 == NULL);

	json5_value_set_int (item, 45457);
	assert (item -> type == JSON5_TYPE_INT);

	item2 = json5_value_set_prop (&value, "somkey44", 8, 0);
	assert (item2 != item);
	assert (item2 -> type == JSON5_TYPE_NULL);
	assert (value.len == 2);
